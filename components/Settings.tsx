...
const emptyUser: Omit<User, 'id'> = { name: '', email: '', role: 'Accountant' };

const UserForm: React.FC<{ user: User | Omit<User, 'id'> | null, onSave: (user: any) => void, onCancel: () => void }> = ({ user, onSave, onCancel }) => {
                    ...
                    {users.map(user => (
                        <tr key={user.id} className="bg-white border-b hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4 text-center">
                               ...
                               <button onClick={() => handleDelete(user.id)} disabled={user.id === '1'} className="font-medium text-red-600 hover:underline disabled:text-gray-400 disabled:cursor-not-allowed">{t('delete')}</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
