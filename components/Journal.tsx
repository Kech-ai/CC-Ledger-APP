            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl p-8 m-4" onClick={e => e.stopPropagation()}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <p><strong>{t('createdBy')}:</strong> {entry.createdBy}</p>
                ...
                <table className="w-full text-sm text-left text-gray-500">
                    ...
                    <tbody>
                        {filteredEntries.map(entry => (
                            <tr key={entry.id} className="bg-white border-b hover:bg-gray-50 cursor-pointer" onClick={() => setSelectedEntry(entry)}>
                                    ...
                                    <td className="px-6 py-4 text-center" onClick={e => e.stopPropagation()}>
                                        {entry.status === 'Pending' ? (
                                            <div className="flex justify-center space-x-2">
                                                <button onClick={() => updateJournalEntryStatus(entry.id, 'Approved')} className="text-green-600 hover:text-green-900 font-medium">{t('approve')}</button>
                                                <button onClick={() => updateJournalEntryStatus(entry.id, 'Rejected')} className="text-red-600 hover:text-red-900 font-medium">{t('reject')}</button>
                                            </div>
                                        ) : '-'}
                                    </td>
                                )}
