...
const TrialBalance: React.FC = () => {
    const { journalEntries, currency, chartOfAccounts } = useAppContext();
    ...
    }, [journalEntries, t, chartOfAccounts]);
        ...
        <table className="w-full text-sm text-left text-gray-500">
            ...
            <tfoot>
                <tr className="font-semibold text-gray-900 bg-gray-50">
                    ...
                    <td className={`px-6 py-3 text-right font-mono ${Math.abs(data.totalDebit - data.totalCredit) > 0.01 ? 'text-red-500' : ''}`}>{formatCurrency(data.totalDebit, currency)}</td>
                    <td className={`px-6 py-3 text-right font-mono ${Math.abs(data.totalDebit - data.totalCredit) > 0.01 ? 'text-red-500' : ''}`}>{formatCurrency(data.totalCredit, currency)}</td>
                </tr>
            </tfoot>
        </table>
...
const ReportLayout: React.FC<{title: string, reportKey: string, reportData: any, children: React.ReactNode}> = ({title, reportKey, reportData, children}) => {
    ...
    const handleAnalyze = async () => {
        ...
        setAiSummary('');
        try {
            const summary = await api.summarizeReport(title, reportData);
