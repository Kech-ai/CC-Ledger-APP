...
const NewJournalEntry: React.FC = () => {
    const { addJournalEntry, currency, chartOfAccounts } = useAppContext();
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            try {
                const { path } = await api.uploadReceipt(formData);
                setReceiptPath(path);
            } catch (err) {
    ...
    const handleScanReceipt = useCallback(async () => {
        ...
        setError(null);
        try {
            const result = await api.scanReceipt(receiptPath);
    ...
    useEffect(() => {
        const fetchSuggestion = async () => {
            if (debouncedDescription) {
                ...
                try {
                    const suggestion = await api.suggestAccount(debouncedDescription);
                             ...
                             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
                                <div className="relative">
                                    ...
                                    <select id="debitAccount" value={debitAccount} onChange={(e) => setDebitAccount(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-cctech-primary focus:ring-cctech-primary sm:text-sm">
                                        ...
                                        {chartOfAccounts.filter(a => a.type === 'Expense' || a.type === 'Asset').map(acc => <option key={acc.code} value={acc.code}>{acc.code} - {t(`account_${acc.code}` as any)}</option>)}
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select id="creditAccount" value={creditAccount} onChange={(e) => setCreditAccount(e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-cctech-primary focus:ring-cctech-primary sm:text-sm">
                                        <option value="">{t('selectAccount')}</option>
                                        {chartOfAccounts.filter(a => a.type === 'Asset' || a.type === 'Liability').map(acc => <option key={acc.code} value={acc.code}>{acc.code} - {t(`account_${acc.code}` as any)}</option>)}
                                    </select>
                                </div>
                             </div>
