...
const RecentTransactions: React.FC = () => {
    ...
    return (
        <div>
            {recent.map(entry => (
                <div key={entry._id} className="flex justify-between items-center py-3 border-b last:border-b-0">
                    <div>
...
const Dashboard: React.FC = () => {
    const { currentUser, journalEntries, currency, chartOfAccounts } = useAppContext();
    ...
    const expenseByCategory = useMemo(() => {
        ...
        const expenseAccounts = chartOfAccounts.filter(a => a.type === 'Expense').map(a => a.code);
